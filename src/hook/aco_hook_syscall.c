...
static aco_share_stack_t *share_stack = NULL;
static aco_t *main_co = NULL;
...
static select_pfn_t g_sys_select_func = NULL;
...
static pthread_getspecific_pfn_t g_sys_pthread_getspecific_func
            = (pthread_getspecific_pfn_t)dlsym(RTLD_DEFAULT,"pthread_getspecific");

static pthread_setspecific_pfn_t g_sys_pthread_setspecific_func
            = (pthread_setspecific_pfn_t)dlsym(RTLD_DEFAULT,"pthread_setspecific");

static pthread_rwlock_rdlock_pfn_t g_sys_pthread_rwlock_rdlock_func
            = (pthread_rwlock_rdlock_pfn_t)dlsym(RTLD_DEFAULT,"pthread_rwlock_rdlock");

static pthread_rwlock_wrlock_pfn_t g_sys_pthread_rwlock_wrlock_func
            = (pthread_rwlock_wrlock_pfn_t)dlsym(RTLD_DEFAULT,"pthread_rwlock_wrlock");

static pthread_rwlock_unlock_pfn_t g_sys_pthread_rwlock_unlock_func
            = (pthread_rwlock_unlock_pfn_t)dlsym(RTLD_DEFAULT,"pthread_rwlock_unlock");
*/
...
static void poll_event_handler(zl_loop_t *loop, int fd, uint32_t events, void *udata);
static void poll_timeout_handler(zl_loop_t *loop, int id, void *udata);
...
ssize_t read(int fd, void *buf, size_t nbyte)
{
    ...
    if (readret < 0)
    {
        LLOG(LL_ERROR, "CO_ERR: read fd %d ret %ld errno %d poll ret %d timeout %d",
...
static int co_poll_inner(zl_loop_t *ctx, struct pollfd fds[], nfds_t nfds, int timeout);
...
int poll(struct pollfd fds[], nfds_t nfds, int timeout)
{
    ...
    return co_poll_inner(zl_loop_get_ct(), fds, nfds, timeout);
}

static int co_select_inner(zl_loop_t *loop, int nfds, fd_set *readfds, fd_set *writefds,
                           fd_set *exceptfds, struct timeval *timeout);
           ...
           fd_set *exceptfds, struct timeval *timeout)
{
    ...
    return co_select_inner(zl_loop_get_ct(), nfds, readfds, writefds, exceptfds, timeout);
}
...
struct hostent *gethostbyname_hook(const char *name)
{
...
res_state __res_state_hook()
{
    ...
    return &(aco_get_co()->res_state_wrap);
}
*/
int __poll_hook(struct pollfd fds[], nfds_t nfds, int timeout)
{
...
static void destroy_co(zl_loop_t *loop, int64_t status, void *udata)
{
    struct co_data *d = udata;
    aco_destroy(d->aco);
    if (d->after_func)
        d->after_func(d->udata);
    free(d);
}

static void co_entry()
{
    struct co_data *d = aco_get_arg();
    d->func(d->udata);
    zl_defer(zl_loop_get_ct(), destroy_co, 0, d);
    aco_exit();
}

void run_co(co_cb func, co_cb after_func, void *udata)
{
    struct co_data *d = malloc(sizeof(struct co_data));
    d->func = func;
    d->after_func = after_func;
    d->udata = udata;
    d->aco = aco_create(aco_get_co() ?: main_co, share_stack, 0, co_entry, d);
    aco_resume(d->aco);
}

int co_poll_inner(zl_loop_t *loop, struct pollfd fds[], nfds_t nfds, int timeout)
{
    if (timeout == 0)
    {
        return g_sys_poll_func(fds, nfds, timeout);
    ...
    struct poll_data *pdata = malloc(sizeof(struct poll_data));
    pdata->fds = malloc(nfds * sizeof(struct pollfd));
    memcpy(pdata->fds, fds, sizeof(struct pollfd) * nfds);
    pdata->nfds = nfds;

    int i, j, n;
    for (i = 0; i < nfds; ++i) {
        int eevents = 0;
        pdata->fds[i].revents = 0;
        if (fds[i].events & POLLIN)
            eevents |= EPOLLIN;
        if (fds[i].events & POLLOUT)
            eevents |= EPOLLOUT;
        int op = EPOLL_CTL_ADD;
        if (i > 0 && pdata->fds[i - 1].fd == fds[i].fd)
            op = EPOLL_CTL_MOD;
        //LLOG(LL_TRACE, "epoll %s fd=%d events=%x", (op == EPOLL_CTL_ADD) ? "add" : "mod", fds[i].fd, eevents);
        n = zl_fd_ctl(loop, op, fds[i].fd, eevents, poll_event_handler, pdata);
        if (n) {
            while (i >= 1) {
                //LLOG(LL_TRACE, "epoll error del fd=%d", fds[i].fd);
                zl_fd_ctl(loop, EPOLL_CTL_DEL, fds[i - 1].fd, 0, NULL, NULL);
                --i;
            }
            goto out;
        }
    ...
    pdata->timer_id = zl_timer_start(loop, timeout, 0, poll_timeout_handler, pdata);

    pdata->aco = aco_get_co();
    aco_yield();

    zl_timer_stop(loop, pdata->timer_id);
    n = 0;
    for (i = 0; i < nfds; ++i) {
        fds[i].revents = pdata->fds[i].revents;
        //LLOG(LL_TRACE, "epoll del fd=%d", fds[i].fd);
        if (i == 0 || fds[i - 1].fd != fds[i].fd)
            zl_fd_ctl(loop, EPOLL_CTL_DEL, fds[i].fd, 0, NULL, NULL);
        if (fds[i].revents)
            ++n;
    }
...
void poll_event_handler(zl_loop_t *loop, int fd, uint32_t events, void *udata)
{
    struct poll_data *pdata = udata;
    int revents = 0;
    if (events & EPOLLIN)
        revents |= POLLIN;
    if (events & EPOLLOUT)
    ...
    for (i = 0; i < pdata->nfds; ++i) {
        if (pdata->fds[i].fd == fd && (pdata->fds[i].events & revents))
            pdata->fds[i].revents = revents;
    }
    zl_timer_again(loop, pdata->timer_id, 0, 0);
}

void poll_timeout_handler(zl_loop_t *loop, int id, void *udata)
{
    struct poll_data *pdata = udata;
    aco_resume(pdata->aco);
}

int co_select_inner(zl_loop_t *loop, int nfds, fd_set *readfds, fd_set *writefds,
                    fd_set *exceptfds, struct timeval *timeout)
{
    ...
    int ts = timeout->tv_sec * 1000 + timeout->tv_usec / 1000;
    int ret = co_poll_inner(loop, fds, n, ts);
    if (ret > 0) {
        ret = 0;
        if (readfds)
            FD_ZERO(readfds);
...
{
    aco_thread_init(0);
    share_stack = aco_share_stack_new(4096 << 10);
    ...
    g_sys_select_func = (select_pfn_t)dlsym(RTLD_NEXT, "select");
